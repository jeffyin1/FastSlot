...
contract Fastsolt {    
    ...
    function play_game()public payable{
        ...
        uint cost = msg.value;
        require(address(this).balance>=10*cost, "contract balance less!!");
        require(address(this).balance>=10*payment, "contract balance less!!");
        require(msg.value <= payment*10 && msg.value >= 0, "value error!!");
        randonNumber = rand()%maxNumber;

        oddsNumber = odds[randonNumber];
        if(oddsNumber >= 1 && oddsNumber <= 10){
